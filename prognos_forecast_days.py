...
async def forecast_days_temperature(query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        temperatures_max = []
        temperatures_min = []# Move initialization outside the loop
...
async def forecast_days_wind_speed(query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        plt.plot(wind_speeds, color='red', linewidth=2, linestyle='solid', label='Максимальна температура',  marker = 'o',markevery = 1)
...
async def forecast_days_pres(query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        plt.plot(pressures, color='red', linewidth=2, linestyle='solid', label='Максимальна температура',  marker = 'o',markevery = 1)
